...
export default function HeatingPage() {
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-14 px-6 py-10">
      <HeroSection eyebrow={service.location} title={service.title} body={service.intro} primaryLabel={service.cta.primaryLabel} primaryHref={service.cta.primaryHref} />
      <section className="space-y-4 rounded-3xl bg-white p-8 shadow-sm">
        <h2 className="font-[family-name:var(--font-heading)] text-3xl font-bold">Heating Service Overview</h2>
        {service.overview.map((paragraph) => <p key={paragraph} className="text-slate-700">{paragraph}</p>)}
      </section>
      <section className="grid gap-6 md:grid-cols-2">
        <div className="rounded-3xl bg-white p-8 shadow-sm"><h2 className="font-[family-name:var(--font-heading)] text-3xl font-bold">Warning Signs</h2><ul className="mt-4 space-y-3 text-slate-700">{service.signs.map((item) => <li key={item}>• {item}</li>)}</ul></div>
        <div className="rounded-3xl bg-white p-8 shadow-sm"><h2 className="font-[family-name:var(--font-heading)] text-3xl font-bold">Benefits</h2><ul className="mt-4 space-y-3 text-slate-700">{service.benefits.map((item) => <li key={item}>• {item}</li>)}</ul></div>
      </section>
