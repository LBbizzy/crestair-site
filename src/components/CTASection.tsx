...
export function CTASection({ heading, body, primaryLabel, primaryHref, primaryId, secondaryLabel, secondaryHref, secondaryId }: CTA) {
  return (
    <section className="rounded-3xl bg-[#041B34] px-6 py-12 text-white md:px-10">
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row">
        ...
        {secondaryLabel && secondaryHref ? <Link data-cta-id={secondaryId} href={secondaryHref} className="inline-flex rounded-full border border-white/20 px-6 py-3 font-semibold text-white">{secondaryLabel}</Link> : null}
      </div>
    </section>
  );
}
